...
class ePhysModule(AbstractModule):
    ...
    def _update_from_tree_HEKA(self, item):
        ...
        try:
            title = "Group %s, Series %s, %s, %s @ %s" % (int(item.group)+1, int(item.series)+1, str(item.label), int(item.sweep), os.path.basename(self.file_path))
        except TypeError:
            title = "Series %s, %s @ %s" % (int(item.series) + 1, str(item.label), os.path.basename(self.file_path))
        self._plot_data(series_name=title)

        dv = (max(self.v_data) - min(self.v_data)) * 0.1
        self.widgets.add_chart_data(self.voltage_chart, x_data=self.x_data, y_data=self.v_data,
                                    series_name='Voltage',
                                    x_range=(0, max(self.x_data)), y_range=(min(self.v_data)-dv, max(self.v_data)+dv),
    ...
    def generate_view_HEKA(self):
        ...
        for i in range(self.data.get_channels()):
            for element in channel_nodes:
                if all([not (element.series is None), not element.trace]):
                    ...
                    try:
                        item.label = self.data.filter_items(group=item.group, series=item.series, sweep=0, trace=item.trace)[0].label
                        node = self.tree.new_node(item.label, '', parent=element, marker=item)
                        node.group, node.series, node.sweep, node.trace = item.group, item.series, item.sweep, item.trace
                        channels.append(node)
                    except IndexError:
                        d = "N/A %s" % str(i + 1)
        ...
        self.widgets.toolbar_add(self.toolbar, self._add_vertical_cursor, arguments=None, img='AB-line')
